...
return ( <div className="p-6 space-y-6"> <div className="flex items-center justify-between"> <h1 className="text-2xl font-bold">Builder</h1> <div className="flex items-center gap-3"> <label className="flex items-center gap-2 text-sm">
\<input
type="checkbox"
checked={usePublished}
onChange={(e) => setUsePublished(e.target.checked)}
/> <span>/map は公開版を使用</span> </label> <button
         onClick={undo}
         disabled={!canUndo}
...
全選択 </button> <button
         ...
         className="px-3 py-2 rounded-lg border"
       >
ステータス初期化 </button> <div className="flex flex-col items-end gap-1"> <button
           onClick={publishAll}
           className="px-3 py-2 rounded-lg bg-black text-white dark:bg-white dark:text-black"
         >
Publish All </button> <div className="text-xs text-zinc-500 flex items-center gap-1"> <span>📅 最終公開:</span> <span>
{publishedAt
? new Date(publishedAt).toLocaleString("ja-JP")
: "未公開"} </span> </div> </div> <Link href="/map" className="px-3 py-2 rounded-lg border">
/map </Link> <Link href="/map?preview=1" className="px-3 py-2 rounded-lg border">
/map?preview=1 </Link> </div> </div>
