...
export default function ArrangePage() {
  ...
  const onFit = () => {
    ...
    const positions = nodes.map(
      (n) => nodePos[n.id] ?? n.position ?? { x: 0, y: 0 }
    )
    ...
    <div className="space-y-3 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-sm text-gray-600">
          Arrange prefectures (Smart guides / Resize / Layers)
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            className="rounded-md border px-2 py-1 text-sm"
            onClick={() => lockToggle(true)}
          >
            🔒 ロック
          </button>
          <button
            className="rounded-md border px-2 py-1 text-sm"
            onClick={() => lockToggle(false)}
          >
            🔓 ロック解除
          </button>
          <button
            className="rounded-md border px-2 py-1 text-sm"
            onClick={savePositionsToBuilder}
          >
            位置/サイズを保存
          </button>
          <Link
        ...
        <ZoomPanCanvas onFitRequest={onFit}>
          ...
          {nodes.map((n: any, i: number) => (
            <DraggableNode
              ...
              initial={
                n.position ?? { x: (i % 8) * 160, y: Math.floor(i / 8) * 120 }
              }
              status={n.status}
