// append-only: simple reconciliation matcher
export type RecoRow = { id: string; amount: number; memo?: string }
export type RecoMatch = { leftId: string; rightId: string; score: number; reason: string[] }

const tokenize = (s = '') =>
  Array.from(new Set(s.toLowerCase().replace(/[^\p{L}\p{N}]+/gu, ' ').trim().split(/\s+/))).filter(Boolean)
