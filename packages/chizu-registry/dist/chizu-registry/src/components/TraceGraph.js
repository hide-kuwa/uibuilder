...
export function TraceGraph({ highlightPath = [] }) {
    ...
    return (_jsxs("div", { className: "relative", style: { padding: 8 }, children: [_jsx("div", { className: "grid gap-4", style: { gridTemplateColumns: `repeat(${colCount}, minmax(0,1fr))` }, children: groups.map((g) => (_jsxs("div", { className: "space-y-2", children: [_jsx("div", { className: "text-xs font-semibold opacity-70", children: g.gid }), _jsx("div", { className: "flex flex-col gap-3", children: g.ids.map((id) => {
                                const isHL = highlightPath.includes(id);
                                const meta = data.nodes[id];
                                return (_jsxs("div", { className: `px-2 py-2 rounded border ${isHL ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white'}`, children: [_jsx("div", { className: `text-xs ${isHL ? 'font-semibold' : ''}`, children: meta?.label ?? id }), _jsx("div", { className: "text-[10px] opacity-60", children: id })] }, id));
                            }) })] }, g.gid))) }), _jsx("svg", { width: "100%", height: svgH, viewBox: `0 0 ${svgW} ${svgH}`, className: "pointer-events-none absolute inset-0", children: data.edges.map((e, idx) => {
                    const a = nodeCenter(e.from);
                    const b = nodeCenter(e.to);
                    const key = `${e.from}->${e.to}`;
                    const hl = hpairs.has(key);
                    return (_jsx("line", { x1: a.x, y1: a.y, x2: b.x, y2: b.y, stroke: hl ? '#2563eb' : '#94a3b8', strokeWidth: hl ? 3 : 1.5, strokeOpacity: 0.9 }, idx));
                }) })] }));
}
