...
function DevActionsPageContent() {
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = (useActionsStore as any).subscribe(
      (state: StoreSnapshot) => ({ list: state.list, presets: state.presets, currentId: state.currentId }),
      persist,
    )
...
export default function DevActionsPage() {
  return (
    <Suspense fallback={<div className="flex min-h-screen items-center justify-center bg-neutral-950 text-neutral-300">Loading...</div>}>
      <DevActionsPageContent />
    </Suspense>
  )
}
